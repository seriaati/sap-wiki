"""
Dump all method names + fields for minion-related classes (MinionConstants,
IMinionBuilder, BoardBuilder, etc.) to identify stat-setter method signatures.

Output JSON: { "ClassName": {"image": "...", "fields": [...], "methods": [...]} }
"""
...
OUTPUT   = str(Path.home() / "Documents/GitHub/sap-data-scrape/minion_methods_dump.json")

# Class name fragments to capture (case-insensitive match on class name)
TARGET_FRAGMENTS = [
    "minionconst", "minionbuilder", "boardbuilder", "battlebuilder",
    "minionmodel", "minionconfig", "miniondata", "minionabout",
    "minionshop", "shopminion", "iminionbuilder",
]
# Also capture any class whose name exactly matches these
TARGET_EXACT = {
    "MinionConstants", "BoardBuilder", "BattleBuilder", "IMinionBuilder",
    "MinionModel", "MinionEnum",
}
...
function setupAfterInit(mod) {
    ...
    const classGetInterfaces  = nfn("il2cpp_class_get_interfaces",  "pointer", ["pointer","pointer"]);
    ...
    for (let ai = 0; ai < asmCount; ai++) {
        ...
        for (let ci = 0; ci < n; ci++) {
            ...
            const match = EXACT.has(cname) || FRAGS.some(f => clow.includes(f));
            if (!match) continue;
...
def main():
    ...
    for cls, info in results.items():
        print(f"\n--- {cls} ({info['image']}) parent={info.get('parent')} ---")
